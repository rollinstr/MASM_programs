TITLE Array Sort     (arraySort.asm)
...
EC_1			BYTE	"**EC: This program uses a recursive QuickSort algorithm to sort the array.",0
...
;********************************************************************
;Description:This function partitions an array around a pivot value.
; The pivot value here is the highest element in the array. All elements
; lower than the pivot are in lower indices, and all elements higher 
; than the pivot are in higher indices.
;Recieves:The OFFSET of the array, and the high and low index of the 
;array.
;Returns:The index of the partition value in the eax register
;Preconditions:None
...
Partition PROC
	...
	mov				ecx, [ebp+28]		;high index
	mov				edi, [ebp+32]		;low index
	mov				i, edi			
	dec				i					;i=low-1
	mov				edx, edi			;j=low (low)
	mov				edi, i
	inc				edi

	;Loop through the array, swapping elements if they are smaller than the pivot

	partitionLoop:
	push			[ebp+36]			;array OFFSET
	push			edx					;j
	call			ReturnArrayElement	
	mov				esi, eax			;store array[j]
	push			[ebp+36]			;Array OFFSET
	push			ecx					;pivot
	call			ReturnArrayElement
	cmp				eax, esi			
	jge				swapElements		;array[pivot]>=array[j]
	inc				edx
	cmp				edx, ecx
	jl				partitionLoop
	jmp				lastSwap
...
Partition ENDP
;********************************************************************
;Description:This procedure sorts an array bey partitioning  the array 
;aroudn a pivot, and recursively calling itself for the lower and upper
;halves of the original array.
;Recieves:The OFFSET of the array, the low and high index of the array
;Returns:None
;Preconditions:None
;Registers Changed:None
;********************************************************************
QuickSort PROC
	...
	mov				edi, [ebp+44]	;arrayOFFSET				;
	mov				ebx, [ebp+36]	;high index
	mov				ecx, [ebp+40]	;low index
	cmp				ebx, ecx		
	jle				skip			;continue only if high>low

	;Find the partition index

	push			[ebp+44]		;array OFFSET
	push			ecx				;low
	push			ebx				;high
	call			Partition		;eax=partition index

	;Recursively call QuickSort on the lower half of the array

	dec				eax				;partition-1
	push			edi				;array OFFSET
	push			ecx				;low
	push			eax				;partition-1
	call			QuickSort

	;Recursively call QuickSort on the upper half of the array
